...
class DynamicHystrixCommand<T> extends HystrixCommand<T> {
    ...
    private static HystrixCommand.Setter mapToSetter(Map<String, Object> params, Closure cl) {
        String group = params.group ?: 'grails'
        def setter = HystrixCommand.Setter.withGroupKey(HystrixCommandGroupKey.Factory.asKey(group))
        setter.andCommandKey(HystrixCommandKey.Factory.asKey((params.command ?: cl.getOwner()) as String))
        if (params.threadpool) {
            setter.andThreadPoolKey(HystrixThreadPoolKey.Factory.asKey(params.threadpool as String))
        }
        if (params.timeout) {
